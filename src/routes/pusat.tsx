...
function LoginPusat({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  async function reset(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Password berhasil direset ke standar (123456). Silakan login dengan password standar.");
      setMode("login");
      setPassword("123456");
      setCode("");
    } finally {
        ...
        <CardContent>
          {mode === "login" ? (
            ...
            <form onSubmit={reset} className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor="code">Kode Forgot Password</Label>
                <Input
                  id="code"
                ...
                <p className="text-[11px] text-muted-foreground">
                  Jika kode benar, password otomatis kembali ke standar (123456).
                </p>
              </div>
              <Button type="submit" className="h-11 w-full rounded-xl" disabled={busy}>
...
function WargaTab({
  ...
  async function hapus(id: string, nama: string) {
    if (!window.confirm(`Nonaktifkan warga "${nama}"? Warga akan diarsipkan dan semua data setoran tetap tersimpan 100% aman tanpa dihapus.`))
      return;
    await deleteResident({ data: { token, id } });
    toast.success("Warga dinonaktifkan (data tetap tersimpan aman).");
    onDone();
  }
...
function AturTab({
  ...
  async function ekspor(format: "excel" | "pdf", exportAllYears: boolean = false) {
    setBusy(true);
    try {
      ...
      const yearsToExport = exportAllYears 
        ? [2022, 2023, 2024, 2025, 2026] 
        : [year];

      for (const y of yearsToExport) {
        const data = await exportYear({ data: { token, year: y } });
        
        // 1. Sheet Buku Kas (Gabungan kronologis per bulan dengan subtotal & total saldo)
        const ledgerRows: any[][] = [];
        ledgerRows.push([`BUKU KAS KEUANGAN RT 06/04 JATI PULOGADUNG - TAHUN ${y}`]);
        ledgerRows.push([`Dicetak pada: ${new Date().toLocaleDateString("id-ID")}`]);
        ledgerRows.push([]);
        ledgerRows.push([
          "No",
          "Bulan",
        ...
        let totalYearMasuk = 0;
        let totalYearKeluar = 0;
        let globalNo = 1;

        // Kelompokkan data per bulan (1 - 12)
        for (let m = 1; m <= 12; m++) {
          const mName = namaBulan(m);
          
          // Pemasukan bulan ini
          const mMasuk = data.masuk.filter((item: any) => {
            if (item.period_month === m && item.period_year === y) return true;
            if (item.sent_date) {
              const dt = new Date(item.sent_date);
              if (dt.getFullYear() === y && dt.getMonth() + 1 === m) return true;
            }
            return false;
          });

          // Pengeluaran bulan ini
          const mKeluar = data.keluar.filter((item: any) => {
            if (item.spend_date) {
              const dt = new Date(item.spend_date);
          ...
          // Tulis transaksi pemasukan
          mMasuk.forEach((item: any) => {
            ...
            ledgerRows.push([
              ...
              "",
              item.note || "-",
              `${item.status === "approved" ? "Lunas" : item.status} (${item.method || "tunai"})`,
            ]);
          });

          // Tulis transaksi pengeluaran
          mKeluar.forEach((item: any) => {
            const nominal = Number(item.amount || 0);
            subKeluar += nominal;
            runningBalance -= nominal;
            ledgerRows.push([
              ...
              "",
              nominal,
              item.note ? `${item.purpose} - ${item.note}` : item.purpose || "-",
              item.is_kasbon ? `Kasbon (${item.kasbon_resident_name ?? ""})` : "Pengeluaran Kas",
            ]);
          });

          // Baris Subtotal per Bulan
          if (mMasuk.length > 0 || mKeluar.length > 0) {
            totalYearMasuk += subMasuk;
            totalYearKeluar += subKeluar;
            ledgerRows.push([
              "",
              `SUBTOTAL ${mName.toUpperCase()}`,
              "",
              "",
              `Surplus/Defisit: ${subMasuk - subKeluar >= 0 ? "+" : ""}${rupiah(subMasuk - subKeluar)}`,
              subMasuk,
              subKeluar,
              `Saldo Kas Berjalan s/d ${mName}: ${rupiah(runningBalance)}`,
              "",
            ]);
            ledgerRows.push([]); // baris pemisah kosong
          }
        }
        ...
        ledgerRows.push([
          ...
          "",
          "",
          `Saldo Bersih Tahun ${y}: ${rupiah(totalYearMasuk - totalYearKeluar)}`,
          totalYearMasuk,
          totalYearKeluar,
          `SALDO AKHIR TAHUN ${y}: ${rupiah(runningBalance)}`,
          "",
        ]);
        ...
        wsLedger["!cols"] = [
          ...
          { wch: 12 }, // Tanggal
          { wch: 18 }, // Kategori
          { wch: 30 }, // Nama Warga / Rincian
          { wch: 24 }, // Pemasukan
          { wch: 22 }, // Pengeluaran
          { wch: 42 }, // Keterangan Pengeluaran
          { wch: 22 }, // Status
        ];
        XLSX.utils.book_append_sheet(wb, wsLedger, `Kas ${y}`);

        // 2. Sheet Warga (Semua warga aktif & non-aktif tetap tersimpan lengkap)
        if (data.residents && data.residents.length > 0) {
          const wargaRows: any[][] = [];
          wargaRows.push([`DAFTAR WARGA RT 06/04 JATI PULOGADUNG (HISTORI LENGKAP)`]);
          wargaRows.push([`Semua data warga (aktif maupun yang sudah pindah) tetap tersimpan utuh`]);
          wargaRows.push([]);
          wargaRows.push([
            "No",
            "Nama Warga",
          ...
          data.residents.forEach((r: any, idx: number) => {
            wargaRows.push([
              idx + 1,
              r.name,
              r.address || "-",
              r.active ? "Aktif" : "Non-Aktif (Pindah / Arsip)",
              r.start_year || 2026,
              namaBulan(r.start_month ?? 1),
              tanggalID(r.created_at),
            ]);
          });
          const wsWarga = XLSX.utils.aoa_to_sheet(wargaRows);
          wsWarga["!cols"] = [
            { wch: 6 },
            { wch: 28 },
            { wch: 25 },
          ...
          XLSX.utils.book_append_sheet(wb, wsWarga, `Data Warga (${y})`);
        }
      }

      if (format === "excel") {
        const filename = exportAllYears
          ? `Laporan-Kas-RT-0604-Lengkap-Semua-Tahun.xlsx`
          : `Laporan-Kas-RT-0604-${year}.xlsx`;
        XLSX.writeFile(wb, filename);
        toast.success(`File spreadsheet Excel (${filename}) berhasil diunduh.`);
      } else {
        // PDF format
        ...
        const dataSingle = await exportYear({ data: { token, year } });
        
        const masuk = dataSingle.masuk.map((m: any) => [
          m.sent_date || "-",
          m.resident_name || "-",
          `${namaBulan(m.period_month)} ${m.period_year}`,
          rupiah(m.amount),
          m.status,
        ]);
        autoTable(doc, {
          startY: 22,
          head: [["Tanggal", "Nama Warga", "Periode", "Nominal", "Status"]],
          body: masuk,
          styles: { fontSize: 8 },
        });
        const yPos = (doc as unknown as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 10;
        doc.text("Pengeluaran Kas", 14, yPos);
        const keluar = dataSingle.keluar.map((k: any) => [
          k.spend_date || "-",
          k.purpose || "-",
          rupiah(k.amount),
          k.note || "-",
        ...
        autoTable(doc, {
          startY: yPos + 4,
          head: [["Tanggal", "Keterangan Pengeluaran", "Nominal", "Catatan"]],
          body: keluar,
          styles: { fontSize: 8 },
        });
      ...
      toast.error((err as Error).message || "Gagal mengunduh laporan.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-sm">
            <FileDown className="size-4" /> Ekspor Spreadsheet Excel & PDF
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Laporan Excel lengkap mencakup: Bulan, Nama Warga, Pemasukan Iuran, Pengeluaran + Keterangan, Subtotal Bulanan, Saldo Kumulatif, dan Rekap Warga (Aktif & Non-Aktif).
          </p>
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold">Pilih Tahun Tertentu:</Label>
            <SearchSelect
              options={yearOptions().slice().reverse().map((y) => ({ value: String(y), label: String(y) }))}
              value={expYear}
              onChange={setExpYear}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" className="h-11 rounded-xl font-medium" disabled={busy} onClick={() => ekspor("excel", false)}>
              {busy ? <Loader2 className="mr-2 size-4 animate-spin" /> : <FileDown className="mr-2 size-4" />}
              Excel ({expYear})
            </Button>
            <Button variant="outline" className="h-11 rounded-xl font-medium" disabled={busy} onClick={() => ekspor("pdf", false)}>
              PDF ({expYear})
            </Button>
          </div>
          <div className="pt-1">
            <Button 
              ...
              disabled={busy} 
              onClick={() => ekspor("excel", true)}
            >
              {busy ? <Loader2 className="mr-2 size-4 animate-spin" /> : <FileDown className="mr-2 size-4" />}
              Ekspor Lengkap Semua Tahun (2022 - 2026) .xlsx
            </Button>
            <p className="mt-1.5 text-center text-[11px] text-muted-foreground">
              Menghasilkan 1 file Excel dengan sheet terpisah untuk setiap tahun & histori warga.
            </p>
          </div>
        </CardContent>
      </Card>
